...
URLS = [f"https://github.com" for _ in range(40)]

async def fetch_url_async(client: httpx.AsyncClient, url: str, semaphore: asyncio.Semaphore):
    async with semaphore:
        try:
            response = await client.get(url, timeout=10.0)
            print(self_status := f"Éxito: {url} -> Código {response.status_code}")
